...
const RegistriesCreate: React.FC = () => {
  ...
  const handleRegistrySubmit = async (data: RegistriesCreateFormData) => {
    try {
      const response = await axios.post<CreateRegistryApiResponse>(
        "/api/registries",
        {
          ...data,
        }
  ...
  return (
    <Accordion
      className="bg-background-alternate shadow-lg px-4 rounded-lg border"
      type="single"
      collapsible
    >
      <AccordionItem className="border-b-0" value="item-1">
        <AccordionTrigger className="font-semibold text-lg">
          Set Your Goal or Achievement
        </AccordionTrigger>
        <AccordionContent>
          <p>
          ...
          <p className="mb-2">
            <span className="font-bold">Tip:</span> You can use emojis to add a
            fun touch to your goal or achievement!
          </p>
